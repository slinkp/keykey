...
def get_window_list():
    ...
    """
    # XXX TODO: get geometry of each window INCLUDING its frame by doing
    # wmiface frameGeometry id (integer id, not hex)
    # ... returns eg. 650x437+0+872
    # ... negative offset like 650x437+-78+905
    # out = subprocess.check_output(['wmctrl', '-l', '-p', '-x', '-G'])
    # windows = [w.split(None, 9) for w in out.splitlines()]
    # windows = [{'id': w[0], 'desktop': w[1],
    #             'pid': w[2],
    #             'x': int(w[3]), 'y': int(w[4]),
...
def get_active_desktop_id():
    # NOTE wmctrl counts from 0, wmiface counts from 1.
